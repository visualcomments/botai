...
1.  **Политика и педагогические ограничения** для агента — учиться вместе со
    ...
    ([AGENTS.md](AGENTS.md));
2.  **18 подобранных Agent-скиллов**, которые дают агенту методологию для
    ...
    объяснение, разбор заданий, оценку, обратную связь, дополнения,
    онбординг в опенсорс-курс как со-разработчика, работу с корпусом через
    **RAG-индекс** (поиск, установка индекса, координаты «файл · фрагмент
    #N»), **автоматическое получение корпуса** курса по опубликованным
    ссылкам, **русскоязычную подачу материала** (русская терминология и
    перевод цитат рядом с дословным оригиналом), ведение **нескольких курсов
    одновременно** (подпроект на курс) и отчёты
...
## Быстрый старт для новичков (без опыта программирования)

Если вы не знакомы с командной строкой и инструментами разработки, не
пугайтесь: **всю установку и настройку сделает за вас сам агент** — вам
нужно только установить **opencode**, создать пустую папку проекта и
вставить один готовый запрос.

### Шаг 1. Установите opencode

**opencode** — это программа-агент, которая понимает инструкции из файла
`AGENTS.md`. Именно в ней будет жить ваш учебный бот-сокурсник.

- **Windows:** откройте PowerShell (кнопка «Пуск» → введите `PowerShell`),
  вставьте и выполните:
  ```powershell
  powershell -ExecutionPolicy Bypass -Command "irm https://opencode.ai/install.ps1 | iex"
  ```
- **macOS:** откройте Терминал (`Cmd+Space` → «Terminal») и выполните:
  ```bash
  curl -fsSL https://opencode.ai/install | bash
  ```
- **Linux:** откройте терминал и выполните ту же команду:
  ```bash
  curl -fsSL https://opencode.ai/install | bash
  ```
- Альтернатива (если установлен Node.js): `npm install -g opencode-ai`.

Проверка: в том же окне введите `opencode --version` — должна появиться
версия. Актуальные инструкции: https://opencode.ai/docs.

### Шаг 2. Создайте пустую папку проекта

Создайте папку для учебного проекта (например, `Мой-учебный-проект`),
зайдите в неё. В терминале проще всего:
```bash
mkdir Мой-учебный-проект && cd Мой-учебный-проект
```
(На Windows можно просто создать папку через Проводник, а затем в
PowerShell: `cd <путь к папке>`.)

### Шаг 3. Запустите opencode и вставьте готовый запрос

В папке проекта выполните `opencode` и вставьте следующий запрос (это
единственное, что нужно сделать вручную):

> **Запрос для агента:**
> «Я новичок и не знаком с программированием. Помоги мне настроить
> учебного ИИ-сокурсника botai. Сделай всё самостоятельно и объясняй
> каждый шаг простыми словами:
>
> 1. Изучи репозиторий https://github.com/visualcomments/botai (README.md,
>    AGENTS.md и документацию) — это твоя инструкция поведения.
> 2. Скачай этот репозиторий в наш текущий проект и установи botai как
>    **отдельный проект-рабочее пространство** (следуй README: установка,
>    создание структуры курсов/прогресса, настройка инструментов и
>    навыков). На Windows можно обойтись без make — используй
>    `python scripts/cli.py ...`, если `make` недоступен.
> 3. Проверь окружение (“doctor”): если не хватает Python 3 или Git —
>    подскажи, что именно скачать и как установить, шаг за шагом.
> 4. Создай подпроект для моего курса (подскажи, как назвать курс), заполни
>    программу и покажи, как я буду заниматься: что отвечать за сессию,
>    где хранится мой прогресс.
> 5. Проведи со мной короткий вводный опрос (уровень, как давать обратную
>    связь, какие задания оцениваются) и расскажи, как начать первое
>    занятие.
>
> Делай это по очереди и задавай мне вопросы перед каждым важным шагом —
> я не программист.»

### Шаг 4. Что произойдёт дальше

- Агент сам скачает botai, создаст изолированный проект (как требует
  политика в `AGENTS.md` — **ничего не пишется в глобальные настройки**),
- проверит окружение (Git / Python 3; в MS Windows — при недоступности
  `make` использует кроссплатформенные команды `python scripts/cli.py`),
- создаст подпроект вашего курса, познакомит вас с consent-gate и режимами
  работы и объяснит команды: `make help`, `make new-course`,
  `make course-set`, `make progress`.
- После настройки вы просто общаетесь с агентом в opencode: он ведёт
  занятия как **сокурсник** (объясняет, разбирает задания, проверяет
  понимание, ведёт досье прогресса).

### Если что-то пошло не так

- **«opencode не найден»** — переустановите по шагу 1 и откройте **новое**
  окно терминала.
- **Python/Git не установлены** — агент подскажет ссылки; для Windows
  Python: https://www.python.org/downloads/ (поставьте галочку «Add python.exe
  to PATH»), Git: https://git-scm.com/download/win.
- **Хотите подробностей об установке вручную (для опытных)** — читайте
  следующий раздел «Установка: отдельный проект» и раздел «Что нужно
  установить заранее».
- Задайте вопрос в нашем чате с агентом — он обязан отвечать простыми
  словами (правило `AGENTS.md`: «объяснять на уровне ученика», не
  перескакивать вперёд).
...
- создаёт рабочее пространство `courses/`, `progress/`, `dist/` и
  инициализирует git, записав `.gitignore` с `.botai/` **до** первого коммита;
...
| [.agents/skills/](.agents/skills/) | 18 Agent-скиллов (по одному `SKILL.md` на способность, плюс ссылки и шаблоны). Здесь лежат настоящие файлы. |
...
| [scripts/install.py](scripts/install.py) | Установщик: создаёт новый изолированный проект и копирует обвязку в него. Файлы агента (`AGENTS.md`, `agent.md`, скиллы, команды) устанавливаются только в рамках проекта, никогда в глобальные конфиги. Записывает `.botai/install.json` — что и откуда установлено (версия, источник, отпечатки файлов): без этой записи обновление не отличило бы вашу правку от версии обвязки. |
| [scripts/update.py](scripts/update.py) | Самообновление обвязки: `make update`. Пишет только файлы обвязки, никогда — `courses/`, `progress/`, `.botai/`. Локально изменённый файл сохраняется и попадает в отчёт; заменённое копируется в `.botai/backup/`. Есть `--check` (есть ли обновление) и `--dry-run`. |
| [scripts/courses.py](scripts/courses.py) | Получение и обновление курсов: `make course-add`, `make course-update`, `make detect-courses`. Курс обновляется только из того источника, из которого был получен; незакоммиченная работа останавливает обновление, а не удаляется. |
| [docs/updating.md](docs/updating.md) | Как обновлять обвязку и курсы: что перезаписывается, что сохраняется, как откатиться, как поставить обновление по расписанию. |
...
| [.opencode/](.opencode/) | Обвязка opencode: primary-агент `botai` + обучающие сабагенты, слэш-команды, симлинки скиллов. |
| [scripts/pathsafe.py](scripts/pathsafe.py) | Проверка имён и удержание путей: имя курса — это имя, а не путь, и ни один путь не выходит за пределы рабочего пространства (включая симлинки и junction). Используется всеми точками входа. |
| [scripts/botai_core/](scripts/botai_core/) | Ядро v2: контракты (`schemas.py`), запись состояния (`store.py`), импорт дневника v1 (`legacy.py`), проверка путей (`paths.py`). Не вызывает модель, не ходит в сеть и не исполняет код курса. |
| [schemas/v2/](schemas/v2/) | Контракты курса, программы и записи прогресса. Ссылки разрешаются только локально: внешний `$ref` отвергается. |
| [examples/](examples/) | `minimal-course` — небольшой курс, валидируемый собственными контрактами; `legacy-course` — вход для проверки миграции v1 → v2. |
| [requirements-core.lock](requirements-core.lock) | Единственная сторонняя зависимость ядра закреплена с хешами (`pip install --require-hashes`). Сам workspace CLI работает на stdlib, чтобы отсутствующую зависимость можно было продиагностировать. |
| [docs/botai-v2-design.md](docs/botai-v2-design.md) | Дизайн v2 и пошаговый план реализации (PR-01…PR-12): что уже сделано и что ещё нет. |
| [CHANGELOG.md](CHANGELOG.md) | Что изменилось по этапам плана. |
...
├── VERSION                    # версия обвязки (по ней update --check видит обновление)
├── scripts/
...
│   ├── multi-course.md            # несколько курсов одновременно (workspace-модель)
│   ├── open-source-contribution.md # режим со-разработчика опенсорс-курса
│   ├── updating.md                # обновление обвязки и курсов, откат, расписание
...
> Интеграция с каталогом **опциональна и требует настройки**: без переменной
> `EDUCATION_CLUB_CATALOG` сервер запустить нельзя. Сам botai собственного
> каталога не содержит — он умеет получать курс по обычному git-адресу
> (`make course-add COURSE_URL=...`) и без каталога.
...
18 скиллов не пересекаются — по одному на способность — и автоматически
...
Два правила, которые применяются во всех режимах.

**Материал — по-русски.** Термины даются по-русски, а не английскими словами
(*workflow* → «порядок работы», *dataset* → «набор данных»). Освоенные
заимствования, которые склоняются (*алгоритм*, *файл*, *интерфейс*), —
русские слова и допустимы; английское слово латиницей внутри русской фразы —
нет. Цитата из иноязычного источника подаётся с **дословным оригиналом** и
русским переводом, помеченным как перевод:

> «the original text, verbatim»
> — перевод: русский перевод
> `txt/<файл>` · фрагмент #N

Оригинал обязателен: цитата — это доказательство, и проверяется она по
корпусу; перевод проверить нечем, и он никогда не выдаётся за слова автора.
Не переводятся пути, координаты фрагментов, команды, переменные окружения и
библиографические идентификаторы. Подробности — навык
`language-and-translation`.

**Корпус получается автоматически.** Курс, публикующий корпус ссылками
(Google Диск, HTTPS/S3, GitHub Releases, HuggingFace), не готов к занятиям,
пока корпус не установлен: без текстов цитируемый фрагмент нечем подтвердить.
Поэтому `make install` скачивает корпус каждого курса, который его публикует,
а в любой момент это можно повторить:

```bash
make corpus COURSE=slug      # получить корпус курса (индекс + тексты)
python scripts/cli.py corpus --course slug
python scripts/cli.py doctor # покажет состояние корпуса по каждому курсу
```

Загрузка проверяет SHA-256 архива **и каждого файла** до распаковки,
устанавливает атомарно и при несовпадении останавливается. Мёртвую ссылку
агент сообщает, а не обходит: подставлять свою ссылку или собирать «свой»
корпус из случайных источников нельзя — непроверенная подборка создаёт
видимость доказательства. Подробности — навык `corpus-acquisition`.

## Как работает агент

Политика определяет четыре режима:
...
## Обновление обвязки и курсов

Обвязка и курсы обновляются отдельно, и ни одно из обновлений не может
потерять вашу работу.

```bash
make update-check                  # есть ли новая версия обвязки (код 10 = есть)
make update                        # обновить обвязку: политика, скиллы, агенты, скрипты
make course-add COURSE_URL=<git-url>   # получить курс в courses/<slug>/
make course-update COURSE=<slug>   # обновить курс из его собственного репозитория
make detect-courses                # что за курсы, откуда и есть ли правки
```

- **Обновление обвязки** пишет только её файлы и никогда не заходит в
  `courses/`, `progress/`, `.botai/`. Файл, который вы правили сами,
  сохраняется и попадает в строку `сохранены локальные версии`; заменённое
  копируется в `.botai/backup/`.
- **Обновление курса** идёт только из того источника, из которого курс был
  получен. Незакоммиченная работа останавливает обновление (её можно
  закоммитить через `make course-update-commit COURSE=<slug>`), а свои коммиты
  в курсе — это повод для вашего решения, а не для принудительного сброса.
- Своя ссылка на корпус или на репозиторий **не подставляется**: мёртвая ссылка
  или изменившийся источник — это сообщение, а не повод искать замену.
