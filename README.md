...
1.  **Политика и педагогические ограничения** для агента — учиться вместе со
    ...
    ([AGENTS.md](AGENTS.md));
2.  **15 подобранных Agent-скиллов**, которые дают агенту методологию для
    ...
    **RAG-индекс** (поиск, установка индекса, координаты «файл · фрагмент
    #N»), ведение **нескольких курсов одновременно** (подпроект на курс) и
    отчёты ([.agents/skills/](.agents/skills/));
...
| [.agents/skills/](.agents/skills/) | 15 Agent-скиллов (по одному `SKILL.md` на способность, плюс ссылки и шаблоны). Здесь лежат настоящие файлы. |
...
├── .agents/skills/            # 15 образовательных Agent-скиллов (реальные файлы)
